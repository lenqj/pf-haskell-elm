    ...
    case msg of
        ...
        GotRepos res ->
            case res of
                ...
                Ok info -> ({ personalDetails = model.personalDetails, events = model.events, selectedEventCategories = model.selectedEventCategories, repos = info}, Cmd.none)

        SelectEventCategory category ->
            ( { personalDetails = model.personalDetails, events = model.events, selectedEventCategories = EventCategory.set category True model.selectedEventCategories, repos = model.repos }, Cmd.none )

        DeselectEventCategory category ->
            ( { personalDetails = model.personalDetails, events = model.events, selectedEventCategories = EventCategory.set category False model.selectedEventCategories, repos = model.repos}, Cmd.none )
