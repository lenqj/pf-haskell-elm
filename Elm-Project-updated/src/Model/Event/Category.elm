...
allSelected = SelectedEventCategories{ academicEvent = True, workEvent = True, projectEvent = True, awardEvent = True }
...
set category value current =
    ...
    in 
        case category of
            Academic -> SelectedEventCategories{
                academicEvent = value,
                workEvent = categories.workEvent,
                projectEvent = categories.projectEvent,
                awardEvent = categories.awardEvent }
            Work -> SelectedEventCategories{
                academicEvent = categories.academicEvent,
                workEvent = value,
                projectEvent = categories.projectEvent,
                awardEvent = categories.awardEvent }
            Project -> SelectedEventCategories{
                academicEvent = categories.academicEvent,
                workEvent = categories.workEvent,
                projectEvent = value,
                awardEvent = categories.awardEvent }
            Award -> SelectedEventCategories{
                academicEvent = categories.academicEvent,
                workEvent = categories.workEvent,
                projectEvent = categories.projectEvent,
                awardEvent = value }
...
view model =
    let 
        (SelectedEventCategories categories) = model
    in
        div [] 
        [
            checkbox "Academic" categories.academicEvent Academic,
            checkbox "Work" categories.workEvent Work,
            checkbox "Project" categories.projectEvent Project,
            checkbox "Award" categories.awardEvent Award
        ]
