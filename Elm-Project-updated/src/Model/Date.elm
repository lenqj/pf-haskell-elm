    ...
    case (d1.month, d2.month) of
        (Just month1, Just month2) -> 
            case Basics.compare d1.year d2.year of
                EQ -> Just (abs((monthToInt month1) - (monthToInt month2)))
                LT -> Just (abs(d1.year - d2.year)*12 + (monthToInt month2) - (monthToInt month1))
                GT -> Just (abs(d1.year - d2.year)*12 + (monthToInt month1) - (monthToInt month2))
        (Just _, Nothing) -> Nothing 
        (Nothing, Just _) -> Nothing
        (Nothing, Nothing) -> Just (abs(d1.year - d2.year)*12)
...
compare (Date d1) (Date d2) =
    ...
    (
        case (d1.month, d2.month) of
            (Just month1, Just month2) -> compareMonth month1 month2
            (Just _, Nothing) -> LT
            (Nothing, Just _) -> GT
            (Nothing, Nothing) -> EQ
    ) 
...
monthsBetweenMonths m1 m2 =
    Basics.abs ((monthToInt m1) - (monthToInt m2))
